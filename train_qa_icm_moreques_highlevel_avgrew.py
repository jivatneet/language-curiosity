...
from agents_modify_representation import *
from config import *
from utils import *
...
from clevr_robot_env_1hop import ClevrEnv
from clevr_robot_env_1hop.high_level_env import HighLevelEnv
...
def main(args):
    ...
    is_load_model = False
    is_render = False
    model_path = 'models/qa-icm-moreques-fiveques-sparsegoal1/{}.model'.format(env_id)
    icm_path = 'models/qa-icm-moreques-fiveques-sparsegoal1/{}.icm'.format(env_id)
    ...
    '''
    num_questions = 5 # set according to number of ques you want to sample
    ...
    while True:
        ...
        total_state, total_reward, total_done, total_next_state, total_action, total_int_reward, total_next_obs, total_values, total_log_prob, total_policy, total_int_ext, total_obs_pre_rep, total_obs_post_rep, total_action_ques  = \
            [], [], [], [], [], [], [], [], [], [], [],[], [], []
        all_frames = []
        ...
        while len(indices) < num_questions:
            ...
            if ind not in indices:
                indices.append(ind)
                questions_set.append(total_questions[ind])
        ...
        while num_episodes_pre_update < 10:
            ...
            for idx in range(num_step):
                ...
                for q in range(num_questions):
                    total_action_ques.append(actions)
            ...
            for ind in range(len(obs_pre_rep)):
                for rep in obs_pre_rep[ind]:
                    ...
                    total_obs_pre_rep.append(rep)
                for rep in obs_post_rep[ind]:
                    combine_post_rep.append(rep)
                    total_obs_post_rep.append(rep)
            ...
            combine_post_rep = np.stack(combine_post_rep).reshape([-1, 128, 4, 4])
            # print("STACK TRAIN: ", combine_pre_rep.shape)
            st_ind = 0
            b_size = 128
            tot_int_rew = []
            while st_ind < combine_pre_rep.shape[0]:
                intrinsic_reward = agent.compute_intrinsic_reward(
                        combine_pre_rep[st_ind: st_ind + b_size],
                        combine_post_rep[st_ind: st_ind + b_size],
                        actions_rollout)
                ...
                for rew in intrinsic_reward:
                    tot_int_rew.append(rew)
                # print("INT REW: ", len(intrinsic_reward), intrinsic_reward)

            # calculating avg intrinsic reward over num_questions
            rew_ind = 0
            while rew_ind < len(tot_int_rew):
                int_rew = []
                for ques_ind in range(num_questions):
                    # print(rew_ind + ques_ind)
                    int_rew.append(tot_int_rew[rew_ind + ques_ind])
                rew_ind += num_questions
                append_rew = []
                append_rew.append(sum(int_rew)/ len(int_rew))
                total_int_reward.append(append_rew)

            # print("TOT INT: ", len(total_int_reward))
        ...
        total_obs_pre_rep = np.stack(total_obs_pre_rep).reshape([-1, 128, 4, 4])
        total_obs_post_rep = np.stack(total_obs_post_rep).reshape([-1, 128, 4, 4])
        total_action = np.stack(total_action).transpose().reshape([-1])
        total_action_ques = np.stack(total_action_ques).transpose().reshape([-1])
        ...
        # Step 5. Training!
        #print("TOATL PRE: ", total_obs_pre_rep.shape)
